  ...
  useEffect(() => {
    const calcularMaxIndex = () => {
      if (cardRef.current && wrapperRef.current) {
        ...
        // Asegurar que la última tarjeta se vea completa
        // Restamos 1 para permitir que la última tarjeta se muestre completamente
        const visibleCards = Math.floor(wrapperWidth / totalCardOffset);
        const maxVisibleIndex = Math.max(0, testimonios.length - visibleCards);

        setMaxIndex(maxVisibleIndex);
      }
    };
